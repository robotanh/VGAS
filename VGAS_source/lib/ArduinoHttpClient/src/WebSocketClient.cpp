...
WebSocketClient::WebSocketClient(Client& aClient, const char* aServerName, uint16_t aServerPort)
 : MyClient(aClient, aServerName, aServerPort),
   iTxStarted(false),
   iRxSize(0)
{
}
...
WebSocketClient::WebSocketClient(Client& aClient, const IPAddress& aServerAddress, uint16_t aServerPort)
 : MyClient(aClient, aServerAddress, aServerPort),
   iTxStarted(false),
   iRxSize(0)
{
}
...
int WebSocketClient::endMessage()
{
    ...
    MyClient::write(0x80 | iTxMessageType);

    // the message is masked (0x80)
    // send the length
    if (iTxSize < 126)
    {
        MyClient::write(0x80 | (uint8_t)iTxSize);
    }
    else if (iTxSize < 0xffff)
    {
        MyClient::write(0x80 | 126);
        MyClient::write((iTxSize >> 8) & 0xff);
        MyClient::write((iTxSize >> 0) & 0xff);
    }
    else
    {
        MyClient::write(0x80 | 127);
        MyClient::write((iTxSize >> 56) & 0xff);
        MyClient::write((iTxSize >> 48) & 0xff);
        MyClient::write((iTxSize >> 40) & 0xff);
        MyClient::write((iTxSize >> 32) & 0xff);
        MyClient::write((iTxSize >> 24) & 0xff);
        MyClient::write((iTxSize >> 16) & 0xff);
        MyClient::write((iTxSize >> 8) & 0xff);
        MyClient::write((iTxSize >> 0) & 0xff);
    }
    ...
    MyClient::write(maskKey, sizeof(maskKey));
    ...
    return (MyClient::write(iTxBuffer, txSize) == txSize) ? 0 : 1;
}
...
size_t WebSocketClient::write(const uint8_t *aBuffer, size_t aSize)
{
    if (iState < eReadingBody)
    {
        ...
        return MyClient::write(aBuffer, aSize);
    }
...
int WebSocketClient::parseMessage()
{
    ...
    // are available
    if (MyClient::available() < 2)
    {
        return 0;
    }

    // read open code and length
    uint8_t opcode = MyClient::read();
    int length = MyClient::read();
    ...
    {
        iRxSize = (MyClient::read() << 8) | MyClient::read();
    }
    else
    {
        iRxSize = ((uint64_t)MyClient::read() << 56) | 
                    ((uint64_t)MyClient::read() << 48) | 
                    ((uint64_t)MyClient::read() << 40) | 
                    ((uint64_t)MyClient::read() << 32) | 
                    ((uint64_t)MyClient::read() << 24) | 
                    ((uint64_t)MyClient::read() << 16) | 
                    ((uint64_t)MyClient::read() << 8)  |
                    (uint64_t)MyClient::read(); 
    }
...
{
    int readCount = MyClient::read(aBuffer, aSize);
