...
// FIXME Update tempToPachube example to calculate Content-Length correctly
    ...
    MyClient(Client& aClient, const char* aServerName, uint16_t aServerPort = kHttpPort);
    MyClient(Client& aClient, const String& aServerName, uint16_t aServerPort = kHttpPort);
    MyClient(Client& aClient, const IPAddress& aServerAddress, uint16_t aServerPort = kHttpPort);
